...
export default function Agenda() {
  ...
  async function saveEvent() {
    ...
    try {
      ...
      setShowModal(false);
      setEditing(false);
      setForm({ id: "", entity_id: "", agenda_type: "consulta", scheduled_for: "", duration_minutes: 60, status: "scheduled", notes: "", resourceId: "1" });
      await load();
    } catch (e: any) {
      setError(String(e.message || e));
    } finally {
  ...
  function handleEdit(event: CalendarEvent) {
    const raw = event.raw as EventItem; // Cast to EventItem
    setForm({
      id: raw.id,
  ...
  async function onDeleteEvent(event: CalendarEvent) {
    ...
    setLoading(true);
    try {
      await apiPost("/events/delete", { id: event.id });
      await load();
    } catch (e: any) {
      setError(String(e.message || e));
    } finally {
    ...
    <div className="flex flex-col h-[calc(100vh-110px)] animate-in-fade overflow-hidden">
      ...
      <div className="flex justify-between items-center bg-black/40 p-4 rounded-xl border border-white/5 mb-4 shrink-0">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => { setEditing(false); setForm({ id: "", entity_id: "", agenda_type: "consulta", scheduled_for: "", duration_minutes: 60, status: "scheduled", notes: "", resourceId: "1" }); setShowModal(true); }} className="liquid-btn cursor-pointer px-4 py-2 bg-primary hover:bg-primary/90 text-primary-foreground font-semibold shadow-[0_0_15px_rgba(var(--primary),0.3)] transition-all flex items-center gap-2">
      ...
      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent className="sm:max-w-md bg-black/95 backdrop-blur-2xl border-primary/20 shadow-2xl p-6">
          ...
          <div className="flex justify-between items-center gap-2 pt-2 border-t border-white/5 mt-4">
            {editing ? (
              <button
                onClick={() => onDeleteEvent(form as unknown as CalendarEvent)}
                className="liquid-btn text-xs px-4 py-2 !bg-rose-500/10 !text-rose-500 hover:!bg-rose-500 hover:!text-white border-rose-500/20 transition-colors"
              >
                Excluir
              </button>
            ) : <div />}
